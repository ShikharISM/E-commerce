          ...
          cartproducts.map((items, ind) => {
            ...
            return (
              <div key={ind} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] items-center gap-4'>
                ...
                <input onChange={(e) => updateCart(items._id, items.size, parseInt(e.target.value)) } className='border max-w-10 px-1 text-center cursor-pointer' type="number" min={1} value={items.quantity} />
                <img onClick={(e) => updateCart(items._id, items.size, 0)} src={assets.bin_icon} className='w-4 mr-4 cursor-pointer' alt="" />
              </div>
            )
          })
        }
